...
from src.position import position_chessboard


def test_position_chessboard_success(
    sample_3d_axis,
    sample_angles,
    ...
    with mock.patch(
        ...
        mock.patch(
            'cv2.waitKey',
            side_effect=[
                ord('a'), ord('c'), ord('f'), ord('i'), ord('m'), ord('n'), 27
            ]
        ) as mock_waitkey, \
        mock.patch(
            'cv2.rotate',
        ...
        mock.patch(
            'src.position.generate_object_points',
            return_value=sample_corners
        ), \
        mock.patch(
            'src.position.generate_3d_axis',
            return_value=sample_3d_axis
        ), \
        mock.patch('src.position.draw_corners'), \
        mock.patch('src.position.add_text_xyz'), \
        mock.patch('src.position.draw_axis_3d'), \
        mock.patch('src.position.draw_principal_point'), \
        mock.patch(
            'src.position.tvecs_to_xyz',
            return_value=sample_distances
        ), \
        mock.patch(
            'src.position.rvecs_to_xyz_euler',
            return_value=sample_angles
        ), \
        mock.patch(
            'src.position.is_within_tolerance',
            return_value=True
        ), \
        mock.patch(
            'src.position.generate_file_name',
            return_value='pose_jpg_of_chessboard_2024_10_31__10_10_10.jpg'
    ):
        ...
        assert mock_waitkey.call_count == 7
